...
const user_seckret_key = seckret_keys.admin_seckret_key;
const service_provider_seckret_key = seckret_keys.admin_seckret_key;
...
const generate_token = (token_data: any) => {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            if (token_data.scope == "service_provider") { seckret_key = service_provider_seckret_key }
...
const decode_token = (token: string, type: string, language: string) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            else if (type == "service_provider") { set_seckret_key = service_provider_seckret_key }
...
const verify_token = async (token: string, type: string, language: string) => {
    try {
        ...
        if (decoded.scope == "service_provider") {
            let query: any = {
                service_provider_id: decoded._id,
                access_token: { $ne: null },
                token_gen_at: decoded.token_gen_at
            }
            let projection = { __v: 0 }
