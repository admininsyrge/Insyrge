...
async function getAdminGuide(slug) {
  try {
    const res = await axios.get(`${BASE_URL_USER}/${slug}/admin-guide`, {
...
async function getExtensionsList() {
  try {
    ...
    // 🔥 Filter only where adminGuide exists
    return extensions.filter((ext) => ext.adminGuide);
  } catch (error) {
  ...
  if (!data) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-[#0B1C3D] text-white">
        <h1 className="text-2xl">Admin Guide Not Found</h1>
      </main>
    );
  }
        ...
        <aside className="md:col-span-1 bg-[#101F44]/60 border border-[#1A2C55] rounded-xl p-6 h-fit sticky top-20">
          ...
          <ul className="space-y-3">
            {extensions.map((ext) => (
              <li key={ext.slug}>
                <Link
                  href={`/extensions/${ext.slug}/admin-guide`}
