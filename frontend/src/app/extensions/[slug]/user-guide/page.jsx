...
async function getUserGuide(slug) {
  try {
    const res = await axios.get(`${BASE_URL_USER}/${slug}/user-guide`, {
      headers: { "Cache-Control": "no-store" },
    });
    return res.data.data;
  } catch (error) {
    console.error("User Guide Fetch Error:", error);
    return null;
  }
}
...
async function getExtensionsList() {
  try {
    ...
    return extensions.filter((ext) => ext.userGuide);
  } catch (error) {
    console.log("Extensions Fetch Error", error);
    return [];
  }
}

export default async function UserGuidePage({ params }) {
  ...
  if (!data) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-[#0B1C3D] text-white">
        <h1 className="text-2xl">User Guide Not Found</h1>
      </main>
    );
  }
        ...
        <aside className="md:col-span-1 bg-[#101F44]/60 border border-[#1A2C55] rounded-xl p-6 h-fit sticky top-20">
          ...
          <ul className="space-y-3">
            {extensions.map((ext, i) => (
              <li key={i}>
                <Link
                  href={`/extensions/${ext.slug}/user-guide`}
